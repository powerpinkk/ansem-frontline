...
The pressure line never participates in collision detection or target cancellation. Idle units alternate between waypoints placed on opposite sides of it, while combatants may pursue verified opponents anywhere inside the arena. If a grizzly reaches the Bull King's immediate airspace, the King turns, closes distance and casts a visible ward that repels the intruder toward bear territory. The verified sell remains alive during bear control; only a qualifying buy reversal or sustained buy control can clear it through the separate reclamation rule.
