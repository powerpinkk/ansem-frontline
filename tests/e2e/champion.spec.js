...
async function installMarketRoutes(page) {
    ...
    await page.route('https://ansem-frontline-stream.ansem-frontline.workers.dev/recent', (route) => route.fulfill({ json: { version: 3, tokenMint: route.request().postDataJSON().token.mint, source: 'verified-rpc-history', pools: 1, trades: [] } }));
