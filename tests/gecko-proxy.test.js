...
import { emptyGeckoPayload, parseGeckoProxyUrl } from '../worker/src/gecko-proxy.js';
...
describe('GeckoTerminal proxy routing', () => {
    ...
    it('uses truthful empty payloads when optional history is rate limited', () => {
        expect(emptyGeckoPayload('trades')).toEqual({ data: [] });
        expect(emptyGeckoPayload('ohlcv/minute')).toEqual({ data: { attributes: { ohlcv_list: [] } } });
    });
});
